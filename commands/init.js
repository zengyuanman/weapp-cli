...
const remote = 'https://github.com/zengyuanman/weapp-template.git';
const fs =require('fs')
...
const initAction = async (name, option) => {
    // console.log("initAction -> option", option)
    // 0. 检查控制台是否可以运行`git `，
    if (!shell.which('git')) {
    ...
    // 5. 清理文件
    const deleteDir = ['.git', '.gitignore', 'README.md']; // 需要清理的文件
    const pwd = shell.pwd();
    ...
    // 将用户的配置打印，确认一下是否正确
    console.log('------------------------1',name,path.join(__dirname,'./'));
    // console.log(answers);
